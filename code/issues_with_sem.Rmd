...
sim_sorting_effect <- function(func, reps = 100, size = 52, baseline = 20, mag = 10, sepsis_scale = 1, micro_scale = 1){
...
sim_sorting <- function(func, reps = 100, size = 52, baseline = 20, mag = 10){
  sim_mult <- do.call("rbind", lapply(X = seq(1:reps), FUN = func, 
                                      baseline = baseline, size = size, 
                                      mag = mag))
  sim_mult2 <- lapply(sim_mult, as.numeric)
...
#simulating data ----
sem_env_effects <- function(x, size = 52, baseline = 0.2, mag = 0.1, micro_scale, sepsis_scale){
  ...
  alpha_hands <- ((int_h1 + int_h2 + int_h3) * mag + baseline) * 100
  ...
  alpha_env <- ((int_e1 + int_e2 + int_e3) * mag + baseline) * 100
...
size = 52; baseline = 20; mag = 6; micro_scale = 1; sepsis_scale = 1
  ...
  int_h2 <- c(rep(0, size - round(size / 3)), rep(1, round(size / 3)))
  int_h3 <- c(rep(0, round(size / 4 * 3)), rep(1, size - round(size / 4 * 3)))
  alpha_hands <- (int_h1 + int_h2 + int_h3) * mag + baseline
  beta_hands <- 100 - alpha_hands
  #environment
  int_e1 <- c(rep(0, round(size / 3)), rep(1, size - round(size / 3)))
  int_e2 <- c(rep(0, round(size / 2)), rep(1, size - round(size / 2)))
  int_e3 <- c(rep(0, round(size / 10 * 2)), rep(1, size - round(size / 10 * 2)))
  alpha_env <- (int_e1 + int_e2 + int_e3) * mag + baseline
  beta_env <- 100 - alpha_env
  #this represents the average change in fidelity based on turning an intervention on or off
  change_fid <- ((1 * mag + baseline) / 100) - ((0 * mag + baseline) / 100)
  #simulating fidelity of intervention
  #overall environmental
  fid_cot <- rbeta(size, alpha_env, beta_env)
  fid_floor <- rbeta(size, alpha_env, beta_env)
  ...
  micro_surf <- mv_env[3, ]
  #this represents the change in microbiology based on increasing fidelity across one area by 10%
  change_micro <- (15 + 0.6 * -micro_scale * 15) - (15 + 0.5 * -micro_scale * 15) 
  #hands
  mv_hands <- mapply(FUN = mv_2, x = fid_staff, y = fid_mat, 
                     MoreArgs = list(sigma = sigma_2, mv_scale = micro_scale))
  micro_staff <- mv_hands[1, ]
  ...
  sepsis <- rbeta(size, alpha_sepsis, beta_sepsis)
  #this represents the average change in sepsis based on a decrease in microbiology of 2 log in all areas
  change_sepsis <- ((13 * 5) * sepsis_scale / 2) / max - ((11 * 5) * sepsis_scale / 2) / max
  #creating data frame
  df_ipc <- data.frame(
    #weeks = weeks,
    int_e1 = int_e1,
...
plot(df_ipc$sepsis, type = "l", ylim = c(0, 1), col = "red", xlab = "week", ylab = "proportion")
lines(df_ipc$fid_cot, col = "black")
lines(df_ipc$micro_cot / 15, col = "blue")
